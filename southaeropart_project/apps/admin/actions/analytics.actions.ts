...
function buildAnalyticsSummary(realRevenue = 0, realOrderCount = 0): AnalyticsSummary {
...
export async function getBusinessAnalyticsMetrics(): Promise<AnalyticsSummary> {
  ...
  if (!admin) {
    console.error("[getBusinessAnalyticsMetrics] Unauthorized access attempt");
    return buildAnalyticsSummary(0, 0);
  }
  ...
  try {
    ...
    return buildAnalyticsSummary(realRevenue, realOrderCount);
  } catch (error) {
