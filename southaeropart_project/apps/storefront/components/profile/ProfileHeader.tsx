...
import { User, ShieldCheck, Car, MapPin, Calendar, Globe, Sparkles } from "lucide-react";
...
export function ProfileHeader({ user, addressCount, vehicleCount }: ProfileHeaderProps) {
  ...
  const preferredLanguage = user.metadata?.preferences?.language || "TH";
  const preferredCurrency = user.metadata?.preferences?.currency || "THB";
